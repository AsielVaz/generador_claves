...
class EspiralPaymentController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $payload = $this->buildEspiralPayload($request, $validated, $payment);
        ...
        if (! $response->successful() || ! $checkoutUrl) {
            ...
            return back()
                ...
                ->with('espiral_api_error', $this->formatEspiralErrorDetails($response, $body, $payload))
                ->withInput();
        }

        $payment->update([
            'reference' => 'Espiral '.$payment->unica,
        ]);
    ...
    private function buildEspiralPayload(Request $request, array $validated, Payment $payment): array
    {
        $user = $request->user();
        $amount = round((float) $validated['amount'], 2);
        ...
        return [
            ...
            'webhook' => [
                'redirectUrl' => route('payments.cash.success', $payment->unica),
                'redirectErrorUrl' => route('payments.cash.error', $payment->unica),
                'backPage' => route('payments.cash.create'),
                'redirectData' => [
    ...
    private function formatEspiralErrorDetails($response, mixed $body, ?array $requestPayload = null): string
    {
        ...
        if ($requestPayload) {
            $sections[] = 'JSON enviado a Espiral:';
            $sections[] = Str::limit(
                json_encode($requestPayload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR),
                5000,
                "\n... request truncado ..."
            );
        }
