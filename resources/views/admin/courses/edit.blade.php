<x-layouts.app title="Editar curso" heading="Editar curso">
    <form method="POST" action="{{ route('admin.courses.update', $course) }}" class="max-w-3xl space-y-5 rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
        @csrf
        @method('PUT')

        <div>
            <label class="text-sm font-medium" for="title">Nombre del curso</label>
            <input id="title" name="title" value="{{ old('title', $course->title) }}" required class="mt-2 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
            @error('title') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="text-sm font-medium" for="description">Descripcion</label>
            <textarea id="description" name="description" rows="4" class="mt-2 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">{{ old('description', $course->description) }}</textarea>
        ...
        <div class="grid gap-5 sm:grid-cols-3">
            ...
            <div>
                ...
                <input id="duration_hours" name="duration_hours" type="number" min="1" value="{{ old('duration_hours', $course->duration_hours) }}" required class="mt-2 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
                @error('duration_hours') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>
        </div>

        <label class="flex items-center gap-2 text-sm font-medium">
            <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $course->is_active)) class="rounded border-zinc-300">
            Curso activo
        </label>
